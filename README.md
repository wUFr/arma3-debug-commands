...
These commands were either collected from BIS/Arma wiki, or from random forums on the internet. Sadly, cant credit all the original authors.

Some can/have to be run either LOCAL or SERVER side via debug commands window (Your MP mission needs to have debug mode enabled - ideally just for admins)
...
```sqf
// Rremove all fuel from all active vehicles, or your own.
vehicle player setfuel 0;
```
### Add weapon to player or player(s)
```sqf
// Add weapon via ID to player, or players.
player addweaponglobal "arifle_MX_GL_F";
```
### Set Player Ammo
```sqf
// Set current player, or player(s) ammo by adjusting the number.
...
### Purple Smoke.
```sqf
// Purple Smoke from player or players s(h)itting area.
